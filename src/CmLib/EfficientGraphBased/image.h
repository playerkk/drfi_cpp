...
public:
	...
	image(const int width, const int height, void *dataP);
...
template <class T> image<T>::image(const int width, const int height, void *dataP) {
	w = width;
	h = height;
	data = (T*)dataP;  // allocate space for image data
	access = new T*[h];   // allocate space for row pointers
